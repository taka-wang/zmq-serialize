...
typedef struct _MbTcpMultipleWriteReq MbTcpMultipleWriteReq;
...
struct  _MbTcpMultipleWriteReq
{
  ProtobufCMessage base;
  CmdHeader *cmd_header;
  MbTcpHeader *mb_tcp_header;
  size_t n_requests;
  MbWriteRequest **requests;
};
#define MB_TCP_MULTIPLE_WRITE_REQ__INIT \
 { PROTOBUF_C_MESSAGE_INIT (&mb_tcp_multiple_write_req__descriptor) \
    , NULL, NULL, 0,NULL }
...
typedef void (*MbTcpSingleWriteReq_Closure)
                 (const MbTcpSingleWriteReq *message,
                  void *closure_data);
typedef void (*MbTcpMultipleWriteReq_Closure)
                 (const MbTcpMultipleWriteReq *message,
                  void *closure_data);
...
extern const ProtobufCMessageDescriptor mb_tcp_multiple_write_req__descriptor;
